...
### 1. ⚡️ 高性能流式解析
- **极低内存占用**：不再将整个 JSON 读入内存。通过 `subprocess` 管道对接 Tshark 输出 (`-T fields` CSV模式)，结合 Python 原生 `csv` 模块进行流式解析。
- **无中间文件**：解析过程中不生成体积巨大的临时文件，直接入库。
- **更快的速度**：相比于旧版 JSON 解析，CSV 解析速度提升显著，且无需依赖第三方 `ijson` 库。
...
| :----------- | :---------------------------- | :---------------------------------- |
| **解析流程** | 生成巨大 JSON -> 全量读入内存 | Tshark流 -> 管道 -> CSV -> SQLite   |
